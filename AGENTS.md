...
- **The shell is config.** A `Shell` is a typed JSON document validated by Zod. Anything the agent changes must be expressible as a typed proposal that resolves to a new `Shell` value (or a constrained delta against one).
- **The agent reads the catalog, not the shell.** The LLM-side agent has no ambient access to user state. Its only inputs are (a) the capability catalog the middleware publishes (`GET /api/capabilities` — widgets + tools the agent may touch) and (b) when the user is iterating on the current frame, a structure-only summary of that frame (instance ids, types, placement — never widget contents). It has no DB access, no read of `/api/frames/*`, no view of widget props or tokens. New capabilities only become available to the agent by being added to a registry that the catalog endpoint exposes.
- **The agent's outputs are proposers and actions.** *Proposers* (`proposeShell`, `proposeWidgetAddition`, …) emit a typed `MutationProposal` that queues for human ratification — they never write. *Action tools* (`web.fetch`, `browser.*`, future portco APIs) are invoked directly, but every action carries a `riskClass` and `riskClass ∈ { write_remote, irreversible }` requires explicit user confirmation in the UI.
- **Widgets are self-describing.** Every widget exports `Component`, `propsSchema` (Zod), `defaultProps`, and `meta` (display name, description). The capability catalog ships `propsSchema` to the agent as JSON Schema.
- **Tools are typed.** Every tool exported to the agent has a Zod input schema, an output type, a `riskClass` (`read` | `write_local` | `write_remote` | `irreversible`), and a `confirm` policy. Tools live under `src/lib/agent/tools/` and are discovered by the catalog endpoint.
